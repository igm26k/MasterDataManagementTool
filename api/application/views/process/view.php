<?php

use yii\helpers\Html;
use yii\helpers\Json;
use yii\widgets\DetailView;

/* @var $this yii\web\View */
/* @var $model app\models\process\Process */

$this->title = $model->name;
$this->params['breadcrumbs'][] = ['label' => Yii::t('app', 'Processes'), 'url' => ['index']];
$this->params['breadcrumbs'][] = $this->title;
\yii\web\YiiAsset::register($this);
?>
<div class="process-view">

    <h1><?= Html::encode($this->title) ?></h1>

    <p>
        <?= Html::a(Yii::t('app', 'Update'), ['update', 'id' => (string)$model->_id], ['class' => 'btn btn-primary']) ?>
        <?= Html::a(Yii::t('app', 'Delete'), ['delete', 'id' => (string)$model->_id], [
            'class' => 'btn btn-danger',
            'data'  => [
                'confirm' => Yii::t('app', 'Are you sure you want to delete this item?'),
                'method'  => 'post',
            ],
        ]) ?>
    </p>

    <?= DetailView::widget([
        'model'      => $model,
        'attributes' => [
            '_id',
            'processId',
            'name',
            [
                'attribute' => 'trigger',
                'format'    => 'raw',
                'value'     => function ($data) {
                    return Json::encode($data->trigger, JSON_UNESCAPED_UNICODE);
                },

            ],
            [
                'attribute' => 'condition',
                'format'    => 'raw',
                'value'     => function ($data) {
                    return Json::encode($data->condition, JSON_UNESCAPED_UNICODE);
                },

            ],
            [
                'attribute' => 'process',
                'format'    => 'raw',
                'value'     => function ($data) {
                    return Json::encode($data->process, JSON_UNESCAPED_UNICODE);
                },

            ],
        ],
    ]) ?>

</div>
